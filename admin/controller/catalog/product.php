...
class ControllerCatalogProduct extends Controller {
	...
	protected function getForm() {
		...
		$this->data['blog_entry_related'] = $this->language->get('blog_related');
		...
		/*Product blog edit*/
		 if (isset($this->request->post['blog_related'])) {
		 	...
		 	$blog = $this->model_catalog_product->getProductblog($this->request->get['product_id']);
		 } else {
		 	$blog = array();
		 }
		 ...
		 foreach ($blog as $product_id) {
		 	$blog_info = $this->model_pavblog_blog->getBlogProduct($product_id);
	...
	public function blogautocomplete() {
		$json = array();

		if (isset($this->request->get['title']) || isset($this->request->get['filter_model']) || isset($this->request->get['filter_category_id'])) {
			$this->load->model('catalog/product');
			$this->load->model('catalog/option');
			$this->load->model('pavblog/blog');
			...
			$results = $this->model_pavblog_blog->getblogs($data);

			foreach ($results as $result) {
				// $option_data = array();

				// $blog_options = $this->model_pavblog_blog->getBlogProduct($result['blog_id']);
				// foreach ($blog_options as $product_option) {
					$json[] = array(
	...
	public function blogautocomplete2() {
		$this->load->model('catalog/product');
		if (isset($this->request->post['blog_related'])) {
			$blog= $this->request->post['blog_related'];
		} elseif (isset($this->request->get['product_id'])) {
			$blog = $this->model_catalog_product->getProductRelated($this->request->get['blog_id']);
		} else {
			$blog = array();
		}
 echo "<pre>";
 print_r($blog);die();
		...
		foreach ($blog as $product_id) {
			$blog_info = $this->model_pavblog_blog->getProductRelated($product_id);
	...
	public function blogautocomplete3($value='')
	{
		$this->load->model('pavblog/blog');
		$data = array(
			'title'=>'camera quan',
