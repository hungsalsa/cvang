      ...
      <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form">
        ...
        <div id="tab-links">
          <table class="form">
            ...
            <!-- Start blog -->
            <tr>
              <td><?php echo $blog_entry_related; ?></td>
              <td><input type="text" name="blogrelated" value="" /></td>
            </tr>
            <tr>
              <td>&nbsp;</td>
              <td><div id="blog-related" class="scrollbox">
                  <?php $class = 'odd'; ?>
                      <!-- Dùng trong edit sản phẩm -->
                <?php foreach ($blog_related as $row) { ?>
                <?php $class = ($class == 'even' ? 'odd' : 'even'); ?>
                <div id="blog-related<?php echo $row['blog_id']; ?>" class="<?php echo $class; ?>"> <?php echo $row['title']; ?><img src="view/image/delete.png" alt="" />
...
//-->
  ...
  /*  Start  autoload blog_product  */

//Blog Related

    $('input[name=\'blogrelated\']').autocomplete({
        delay: 500,
        source: function(request, response) {
            $.ajax({
                url: 'index.php?route=catalog/product/blogautocomplete&token=<?php echo $token; ?>&title=' +  encodeURIComponent(request.term),
                dataType: 'json',
                success: function(json) {
        ...
        select: function(event, ui) {
            ...
            $('#blog-related').append('<div id="blog-related' + ui.item.value + '">' + ui.item.label + '<img src="view/image/delete.png" alt="" /><input type="hidden" name="blog_related[]" value="' + ui.item.value + '" /></div>');
