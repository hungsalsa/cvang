...
class ModelCatalogProduct extends Model {
	...
	public function getBlogRelated($product_id) {
		$product_data = array();

		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_blog_related WHERE product_id = '" . (int)$product_id . "'");
		
		foreach ($query->rows as $key => $result) { 
			$blog_data[] = $result['blog_id'];
		}
